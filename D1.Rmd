...
## table with cell types assigned by SevenBridges
ct1 <- read.csv(paste(data.dir, "_1_Combined_Rhapsody19_Target1_cell_type_experimental.csv", sep = ""))
ct1$Cell_Index <- paste(ct1$Cell_Index, "_S1", sep = "")

## joining both metadata
coldata1 <- left_join(coldata1, ct1, by = "Cell_Index")
...
## table with cell types assigned by SevenBridges
ct2 <- read.csv(paste(data.dir, "Combined_Rhapsody19_Target2_cell_type_experimental.csv", sep = ""))
ct2$Cell_Index <- paste(ct2$Cell_Index, "_S2", sep = "")

## joining both metadata
coldata2 <- left_join(coldata2, ct2, by = "Cell_Index")
...
ggsave("plot1.png", p1, height = 2.5, width = 7.5)
ggsave("plot2.png", p2, width = 7, height = 2)
```
...
ggsave("plot3.png", p3, height = 4, width = 4)
ggsave("plot4.png", p5, height = 4, width=6)
```
...
score$pathway <- case_when(
  ...
  score$markers %in% c("CD45RA.HI100.PTPRC", "CD3D", "CD3E", "CD3G", "CD3.SK7.CD3E", "CD8A", "CD8B", "CD8.SK1.CD8A") ~ "Naive T-cell Markers",
...
p10
ggsave("plot5.png", p10, width = 14, height = 3)
```
