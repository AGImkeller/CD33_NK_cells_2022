...
p1 <- plotTSNE(sce_d1, colour_by="igraph_lbls", other_fields = c("condition")) + 
  ...
  labs(color = "cluster")
p1
ggsave("fig6d.tiff", p1, height = 4, width = 4)
ggsave("fig6d.pdf", p1, height = 4, width = 4)
```
...
p2 <- as.data.frame(colData(sce_d1)) %>%
  ...
  guides(colour = guide_legend(nrow = 1))
p2
ggsave("fig6f.tiff", p2, height = 4, width = 6)
ggsave("fig6f.pdf", p2, height = 4, width = 6)
```
...
```{r}
a <- c("XCL1", "TYMS", "TOP2A", "TNF", "MYC", "IL3", "IFNG", "HMGB2", "GZMA", "FCGR3A", "CSF2", "CCL4", "CCL3", "AURKB")
p5 <- plotDots(sce_d1, features = rev(a), group = "igraph_lbls", color = c("darkblue", "lightblue", "gray", "yellow", "orange", "darkorange", "red", "darkred")) +
...
p5
ggsave("fig6e.tiff", p5, width = 7, height = 7)
ggsave("fig6e.pdf", p5, width = 7, height = 7)
```
...
p8 <- plotExpression(sce_d1_a[,colData(sce_d1_a)$group == "BeforeCoc"], features = c("IFNG", "TOP2A", "AURKB"), x = "condition", ncol = 3, colour_by = "condition") + 
  ...
  theme(axis.text.x=element_blank(), 
        ...
        strip.text.x = element_text(size = 14), axis.text.y = element_text(size = 14)) +
  labs(color = "condition") 
 
p9 <- p8 + 
  p7 +
  plot_layout(guides = "collect", ncol = 1, heights = c(1,1)) &
  theme(legend.position="bottom", legend.box="vertical", legend.margin=margin(), legend.text = element_text(size=14), legend.title = element_blank())
p9
ggsave("fig6g.tiff", p9, width = 7, height = 7)
ggsave("fig6g.pdf", p9, width = 7, height = 7)
```
