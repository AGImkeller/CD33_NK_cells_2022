...
p1 <- plotTSNE(sce_d1, colour_by="igraph_lbls", other_fields = c("condition")) + 
  ...
  labs(color = "cluster")
p1
ggsave("plot6.png", p1, height = 4, width = 4)
```
...
p2 <- as.data.frame(colData(sce_d1)) %>%
  ...
  guides(colour = guide_legend(nrow = 1))
p2
ggsave("plot7.png", p2, height = 4, width = 6)
```
...
p5 <- plotDots(sce_d1, features = a, group = "igraph_lbls", color = c("gray", "yellow", "orange", "red", "darkred", "purple", "lightblue", "blue")) +
...
p9
ggsave("plot8.png", p9, width = 6, height = 6)
```
